...
Section TypeInSet.
Variable (U : finType) (S : {set U}).

Definition TSet : finType := {x : U | x \in S} : subFinType _.
Definition MkTSet (x : U) (xinS : x \in S) : TSet := exist _ x xinS.

Lemma TSetP (x : TSet) : val x \in S.
Proof. by case: x. Qed.
Lemma MkTSetE (x : TSet) : MkTSet (TSetP x) = x.
...
Record bijType := MkBij { carrier : finType; setBij : {set carrier} }.
Coercion elBij (U : bijType) : finType := TSet (setBij U).

Lemma card_setBij (U : bijType) : #|setBij U| = #|U|.
Proof. by rewrite card_TSet. Qed.

Fact idfun_is_bijective T : bijective (@idfun T).
Proof. by exists idfun. Qed.

HB.instance Definition _ :=
  isCategory.Build bijType elBij
    (fun (a b : bijType) (f : a -> b) => bijective f)
    (fun (a : bijType) => @idfun_is_bijective a)
    (fun (a b c : bijType) (f : a -> b) (g : b -> c)
         (fb : bijective f) (gb : bijective g) => bij_comp gb fb).

End FinBijCat.
Notation "'[Bij:'  S ']'" := (MkBij [set: S]).

Definition Bij : category := bijType.
...
Qed.
Lemma finv_id (U : Bij) : finv (@idfun (el U)) =1 idfun.
Proof.
...
Variables (cU cV : finType) (f : cU -> cV) (fbij : bijective f).
Definition fBij_fun : el [Bij: cU] -> el [Bij: cV] :=
  fun x => MkTSet (in_setT (f (val x))).
Fact fBij_bij : bijective fBij_fun.
Proof.
case: fbij => g fK gK.
by exists (fun x => MkTSet (in_setT (g (val x)))) => x;
  apply val_inj; rewrite /= ?fK ?gK.
Qed.
HB.instance Definition _ := BijHom.Build [Bij: cU] [Bij: cV] fBij_fun fBij_bij.
Definition fBij : {hom [Bij: cU] -> [Bij: cV]} := fBij_fun.
...
Variable (U V : Bij) (I : {set carrier U}).
Hypothesis (Iincl : I \subset setBij U).
Variable (f : {hom U -> V}).

Fact upincl_subproof (x : MkBij I) : (\val x) \in setBij U.
Proof. by case: x => x Px /=; apply/(subsetP Iincl). Qed.
Definition upincl (x : MkBij I) : U := MkTSet (upincl_subproof x).
Definition imSubBij := (val \o f \o upincl) @: (MkBij I).
Lemma imSubBijP : imSubBij \subset setBij V.
Proof. by apply/subsetP => u /imsetP[x xin {u}->] /=; apply: TSetP. Qed.
Fact restr_subproof (x : MkBij I) : (val \o f \o upincl) x \in imSubBij.
Proof. exact: imset_f. Qed.
Definition restr (x : el (MkBij I)) : el (MkBij imSubBij) :=
  MkTSet (restr_subproof x).

Fact restr_inj : injective restr.
Proof.
move=> /=[x Hx][y Hy] /= /(congr1 val) /= /val_inj/Bij_injP.
by rewrite /upincl => [[heq]]; apply val_inj.
Qed.
Fact restr_bij : bijective restr.
Proof.
apply: (inj_card_bij restr_inj); apply: eq_leq.
rewrite /= !card_sub /= card_imset ?card_sub //.
by move=> /= x y Heq; apply/restr_inj/val_inj.
Qed.
HB.instance Definition _ := BijHom.Build _ _ restr restr_bij.
Definition restr_hom : {hom[Bij] MkBij I -> MkBij imSubBij} := restr.

End Restriction.


Definition voidB := MkBij [set: void].
Definition unitB := MkBij [set: unit].
Definition boolB := MkBij [set: bool].
Definition negbB : el boolB -> el boolB :=
  fun x => MkTSet (in_setT (negb (val x))).
Fact negbBK : involutive negbB.
Proof. by move=> x; apply: val_inj; rewrite /= negbK. Qed.
HB.instance Definition _ := BijHom.Build boolB boolB negbB (inv_bij negbBK).
...
Definition enum_Bij (x : el U) : el (MkBij [set: 'I_#|U|]) :=
  MkTSet (in_setT (cast_ord (card_setBij U) (enum_rank_in (TSetP x) (val x)))).
Lemma enum_Bij_bij : bijective enum_Bij.
Proof.
apply: inj_card_bij; last by rewrite card_TSet cardsT card_ord.
rewrite /enum_Bij => x y [/val_inj/enum_rank_in_inj eq].
by apply/val_inj/eq; exact: TSetP.
Qed.
HB.instance Definition _ :=
  BijHom.Build U (MkBij [set: 'I_#|U|]) enum_Bij enum_Bij_bij.

Definition cardSp (n : nat) := #|A [Bij: 'I_n]|.
Lemma cardSpE : #|A U| = cardSp #|U|.
Proof. by rewrite (Bij_eq_card (A # enum_Bij)). Qed.
...
Definition isotype U (x : A U) := isorepr ((A # @enum_Bij U) x).
Definition cardiso n := #|isoreprs [Bij: 'I_n]|.
...
Proof.
by rewrite /cardiso card_isoreprE (card_isoclassesE (@enum_Bij U)).
Qed.
Lemma cardiso_ordE n : #|isoclasses [Bij: 'I_n]| = cardiso n.
Proof. by rewrite cardisoE card_TSet /= cardsT card_ord. Qed.

Lemma isotype_mem U (x : A U) : isotype x \in isoreprs [Bij: 'I_#|U|].
Proof. exact: isorepr_mem. Qed.
Lemma isotype_ex U (x : A U) :
  exists f : {hom U -> [Bij: 'I_#|U|]}, (A # f) x = isotype x.
Proof.
rewrite /isotype.
set ix := (A # @enum_Bij U) x.
have [_ H] := orbit_transversalP (actSpP [Bij: 'I_#|U|]).
have /orbitP[/= s _] : isotype x \in orbit (actSp [Bij: 'I_#|U|]) setT ix.
  by apply: (repr_mem_pblock (isotypesP _)); exact: imset_f.
rewrite /actSp_fun /isotype -/ix => <-.
by exists (s \o (@enum_Bij U)); rewrite /ix functor_o.
Qed.

Lemma isotypeE U (x : A U) (f : {hom U -> [Bij: 'I_#|U|]}) :
  (A # f) x \in isoreprs [Bij: 'I_#|U|] -> (A # f) x = isotype x.
Proof.
...
Lemma isotypeP U V (x : A U) (y : A V) :
  reflect (exists f : {hom U -> V}, (A # f) x = y)
    (existT (fun n => A [Bij: 'I_n]) #|U| (isotype x) ==
     existT (fun n => A [Bij: 'I_n]) #|V| (isotype y)).
Proof.
...
apply/setP => b'; apply/orbitP/imsetP => [[/= s _ {b'}<-]|].
  ...
  rewrite mem_imset; last exact: Bij_injP.
  by apply/orbitP; exists s; first by rewrite inE.
...
Proof. by move=> U; apply: (Bij_eq_card (F [Bij: 'I_U])). Qed.

Lemma cardiso_SpE : cardiso A =1 cardiso B.
Proof.
move=> n; rewrite -!cardiso_ordE.
pose ff := fun (S : {set A _}) => F [Bij: 'I_n] @: S.
rewrite -(card_imset (f := ff)); last exact: (imset_inj (Bij_injP _)).
...
Proof. by rewrite /cardSp card_TSet /= cardsT card_ord. Qed.
...
rewrite [RHS](_ : _ = 1%N) //=.
rewrite -[RHS](cards1 [set: idSp [Bij: 'I_n.+1]]); congr #|pred_of_set _|.
apply/setP => /= S; rewrite !inE.
apply/imsetP/eqP => [[i _ {S}->] | {S}->]; first by rewrite isoclass_idSp.
by exists (MkTSet (in_setT ord0)); rewrite ?inE // isoclass_idSp.
Qed.
...
Definition cast_TSet I J (eq : I = J) (y : el (MkBij I)) : el (MkBij J) :=
  ecast X (TSet X) eq y.
...
Lemma Tagged_SpTSet_castE (V : Bij) (I J : {set V}) (eq : I = J) (A : Species) x :
  Tagged (fun U => A U) ((A # cast_TSet eq) x) =
    Tagged (fun U => A U) x.
Proof.
subst I => /=.
...
Proof. by rewrite /cardSp card_TSet /= cardsT card_void. Qed.
...
Proof. by move: x y => /= [[p]][[q]]; apply: val_inj. Qed.
...
Proof. by rewrite /cardSp card_TSet /= cardsT card_unit. Qed.
